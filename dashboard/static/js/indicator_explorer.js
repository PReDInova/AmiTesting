 ...
 * Extracted from partials/indicator_explorer_js.html
 * Handles chart rendering, indicator visualization, signal computation,
 * sidebar controls, parameter sliders, code map, bar analysis, and tooltips.
 *
 * Expects window.EXPLORER_CONFIG to be set inline with Jinja2 values:
 *   - strategyId, symbol, defaultSymbol, indicatorConfigs, paramDefs,
 *     codeMap, rawDescription, rawAflContent
 */
(function() {
    // ── Configuration (from inline Jinja2 config) ──
    var cfg = window.EXPLORER_CONFIG;
    const strategyId = cfg.strategyId;
    let symbol = cfg.symbol;
    let indicatorConfigs = cfg.indicatorConfigs;
    const paramDefs = cfg.paramDefs;
    const codeMap = cfg.codeMap;
    let paramInfo = {};
    let indicatorInfo = {};
    const rawDescription = cfg.rawDescription;
    const rawAflContent = cfg.rawAflContent;
    ...
    function setupSymbolSelector() {
        ...
        sel.addEventListener('change', function() {
            symbol = this.value || cfg.defaultSymbol;
            loadFullData(currentInterval);
        });
    }
